# Description:
# Removes an IAM user from an IAM Group in the requested account
#
# Commands:
# zeke remove IAM user {username} from {groupname} in {account}  - i.e. remove mmclane from LameAdmins in hl-dev.
...
executePowerShell = edge.func('ps', -> ###
  ...
  . .\scripts\remove-awsIAMUserFromGroup.ps1
  # Edge.js passes an object to PowerShell as a variable - $inputFromJS
  # This object is built in CoffeeScript on line 28 below
  remove-awsIAMUserFromGroup -Username $inputFromJS.username -GroupName $inputFromJS.groupname -ProfileName $inputFromJS.profile
###
)

module.exports = (robot) ->
  # Capture the user message using a regex capture to find the name of the service
  robot.respond /remove IAM user (.*) from (.*) in (.*)/i, (msg) ->
    if robot.auth.hasRole(msg.envelope.user, 'awsadmin')
      ...
      callPowerShell = (psObject, msg) ->
        msg.send "You got it!  I'll remove "+username+" from the "+groupname+" group in "+profile+"."
